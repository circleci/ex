...
func (u unifiedQuerier) NamedSelectContext(ctx context.Context, dest interface{}, query string, arg interface{}) error {
	err := u.q.NamedSelectContext(ctx, dest, query, arg)
	if errors.Is(err, sql.ErrNoRows) {
		return ErrNop
	}
	_, err = mapError(err)
